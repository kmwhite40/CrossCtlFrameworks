...
async def ingest_into_sqlite(engine: AsyncEngine, xlsx_path: Path) -> dict:
    ...
    try:
        async with engine.begin() as conn:
            ...
            for spec in FRAMEWORKS:
                await conn.execute(
                    text("INSERT OR IGNORE INTO ccf.frameworks(code,name,family,description) VALUES (:c,:n,:f,:d)"),
                    {"c": spec.code, "n": spec.name, "f": spec.family, "d": spec.description},
                )
            await conn.execute(
                text("INSERT OR IGNORE INTO ccf.frameworks(code,name,family,description) VALUES ('OTHER','Other / Misc','Other','Unclassified columns')")
            )
            ...
            if ASSESSMENT_SHEET in wb.sheetnames:
                ...
                for i, row in enumerate(ws.iter_rows(values_only=True)):
                    ...
                    """), {
                        ...
                        "tst": record.get("TEST"),
                        "l": 1 if str(record.get("FISMA Low") or "").strip().lower() in {"x","yes","y","1"} else 0,
                        "m": 1 if str(record.get("FISMA Mod") or "").strip().lower() in {"x","yes","y","1"} else 0,
                        "h": 1 if str(record.get("FISMA High") or "").strip().lower() in {"x","yes","y","1"} else 0,
                    ...
                    for header, value in record.items():
                        ...
                        await conn.execute(text("""
                            INSERT INTO ccf.framework_mappings(control_id, framework_id, column_key, value)
            ...
            stats["worksheets"] = len(wb.sheetnames) - (1 if ASSESSMENT_SHEET in wb.sheetnames else 0)
    finally:
        wb.close()
    return stats


def run_ingest(dsn: str, xlsx_path: Path) -> dict:
