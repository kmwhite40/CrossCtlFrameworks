"""Coverage heatmap (framework × family) — read-only reporting."""
...
@router.get("/matrix")
async def coverage_matrix(session: AsyncSession = Depends(get_session)) -> dict:
    ...
    fam_totals = {
        r.code: r.n for r in (await session.execute(
    ...
    fw_totals = {
        r.code: r.n for r in (await session.execute(
            select(Framework.code, func.count(func.distinct(FrameworkMapping.control_id)).label("n"))
            .select_from(FrameworkMapping)
            .join(Framework, Framework.id == FrameworkMapping.framework_id)
    ...
    return {
        "cells": [
            {"framework": r.framework, "family": r.family, "controls": r.controls}
            for r in rows
        ],
