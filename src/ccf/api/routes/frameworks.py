...
async def list_frameworks(
    ...
    stmt = (
        select(
            ...
            func.count(FrameworkMapping.id).label("mapping_count"),
        )
    ...
    return [{str(k): v for k, v in r._mapping.items()} for r in rows]
...
async def framework_controls(
    ...
    return {
        ...
        "total": total,
        "items": [{str(k): v for k, v in r._mapping.items()} for r in rows],
    }
