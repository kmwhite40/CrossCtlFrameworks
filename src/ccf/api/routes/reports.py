...
async def _scope_controls(session: AsyncSession, baseline: Baseline | None):
    stmt = (
        select(Control)
...
async def build_report(
    ...
    if framework:
        fw = (await session.execute(
            select(Framework).where(Framework.code == framework.upper())
        )).scalar_one_or_none()
        if not fw:
    ...
    for c in controls:
        ...
        lines.append({
            ...
            "last_assessed_on": impl.last_assessed_on.isoformat() if impl and impl.last_assessed_on else None,
            "crosswalk_framework": fw.code if fw else None,
            "crosswalk_values": "; ".join(
                f"{m.column_key}={m.value}" for m in mappings
            ) or None,
        })
    ...
    # CSV
    buf = io.StringIO()
    writer = csv.DictWriter(buf, fieldnames=list(lines[0].keys()) if lines else [
        "identifier", "family", "control_name"])
    writer.writeheader()
    for row in lines:
        writer.writerow(row)
    buf.seek(0)
