...
Approve and apply are role-gated to the same roles that approve a waiver, and
the requester cannot approve their own plan (enforced in the service, where it
...
#: The roles that may approve or apply a change to an environment -- the same
#: set that approves a waiver.
ENFORCER_ROLES = ("admin", "issm", "isso")
...
async def create(
    ...
    """Build a plan from the latest recorded findings. Writes nothing.
