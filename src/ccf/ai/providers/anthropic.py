...
class AnthropicProvider(AIProvider):
    ...
    async def generate_structured_output(
        ...
        text_resp = await self.generate_text(
            GenerateTextRequest(
        ...
        data = _structured.validate(_structured.extract_json(text_resp.text), request.schema)
        return StructuredGenerationResponse(
            data=data,
            model=text_resp.model,
            input_tokens=text_resp.input_tokens,
            output_tokens=text_resp.output_tokens,
        )
    ...
    async def list_supported_models(self) -> list[ModelDescriptor]:
        ...
        return [
            ModelDescriptor(m["id"], m.get("display_name", m["id"]))
