...
def upgrade() -> None:
    ...
    op.add_column(
        "ingestion_runs",
        sa.Column(
            "workbook_version_id", sa.Integer,
    ...
    op.create_index("ix_ccf_audit_rejected_rows_run", "rejected_rows",
                    ["run_id"], schema="ccf_audit")

    op.create_table(
        "control_history",
        sa.Column("id", sa.BigInteger, primary_key=True),
        sa.Column("identifier", sa.String(128), nullable=False),
        sa.Column("workbook_version_id", sa.Integer,
    ...
    op.create_index("ix_control_history_ident", "control_history",
                    ["identifier"], schema="ccf_audit")
    op.create_index("ix_control_history_ver", "control_history",
                    ["workbook_version_id"], schema="ccf_audit")
    ...
    op.create_index("ix_mapping_history_ident", "mapping_history",
                    ["identifier"], schema="ccf_audit")
    op.create_index("ix_mapping_history_col", "mapping_history",
                    ["column_key"], schema="ccf_audit")
