...
def upgrade() -> None:
    ...
    # --- Enums -------------------------------------------------------------
    fips199 = postgresql.ENUM(
        "low", "moderate", "high", name="fips199_level", schema="ccf", create_type=True
    )
    baseline = postgresql.ENUM(
        "low", "moderate", "high", name="fedramp_baseline", schema="ccf", create_type=True
    )
    ato_status = postgresql.ENUM(
        "none",
        "in_progress",
        "authorized",
        "expired",
    ...
    op.create_table(
        ...
        sa.Column(
            "family_id", sa.Integer, sa.ForeignKey("ccf.control_families.id", ondelete="SET NULL")
        ),
    ...
    op.create_index(
        "idx_controls_audit_payload_gin",
        "controls",
        ["audit_payload"],
        postgresql_using="gin",
        schema="ccf",
    ...
    op.create_table(
        ...
        sa.Column(
            "control_id",
            sa.BigInteger,
            sa.ForeignKey("ccf.controls.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "framework_id", sa.Integer, sa.ForeignKey("ccf.frameworks.id", ondelete="SET NULL")
        ),
    ...
    op.create_index(
        "ix_ccf_framework_mappings_control_id", "framework_mappings", ["control_id"], schema="ccf"
    )
    op.create_index(
        "ix_ccf_framework_mappings_framework_id",
        "framework_mappings",
        ["framework_id"],
        schema="ccf",
    )
    op.create_index(
        "ix_ccf_framework_mappings_column_key", "framework_mappings", ["column_key"], schema="ccf"
    )
    op.execute(
    ...
    op.create_table(
        ...
        sa.Column(
            "worksheet_id",
            sa.Integer,
            sa.ForeignKey("ccf.worksheets.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_index(
        "idx_worksheet_rows_payload_gin",
        "worksheet_rows",
        ["payload"],
        postgresql_using="gin",
        schema="ccf",
    ...
    op.create_index(
        "ix_ccf_worksheet_rows_worksheet_id", "worksheet_rows", ["worksheet_id"], schema="ccf"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "control_id",
            sa.BigInteger,
            sa.ForeignKey("ccf.controls.id", ondelete="RESTRICT"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "control_id", sa.BigInteger, sa.ForeignKey("ccf.controls.id", ondelete="SET NULL")
        ),
        sa.Column("title", sa.String(512), nullable=False),
...
def downgrade() -> None:
    ...
    # Keep the ccf schema so Alembic can update its version table on the way to "base".
