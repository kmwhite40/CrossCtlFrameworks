...
- [ ] **Step 1: Write the failing test.** Table-driven over every bucket and
  both boundaries:
...
- [ ] **Step 3: Implement.** `classify` takes the already-resolved
...
- [ ] **Step 1: Write the failing test** — round-trips; policy defaults match
...
- [ ] **Step 2: Run it.** Expect `ImportError`.
- [ ] **Step 3: Implement.** Waves are built smallest-first: wave 1 gets one
...
- [ ] **Step 1: Write the failing test** — the report; setting a policy changes
...
- [ ] **Step 3: Implement.** Completing a wave is an assertion that work was
...
- [ ] **Step 1:** full suite, `ruff check .`, `mypy src`, `bandit -r src -ll -x tests`,
  `alembic heads`.
- [ ] **Step 2: Mutate every guard.** At minimum: both SLA boundaries; the
...
- [ ] **Step 4: Demonstrate** — ingest a scan, show the SLA report breaching on
